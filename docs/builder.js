...
async function loadBuilderData() {
  const sources = canUseLocalApi()
    ? ["/api/builder-data", "./builder-data.json"]
    : ["./builder-data.json"];
  ...
  for (const source of sources) {
    try {
      ...
      // GitHub Pages has no Express API, so fall through to the static JSON file.
    }
  }

  throw new Error("Could not load builder data.");
}
...
async function refreshProfile() {
  ...
  const staticProfile = enrichStaticProfile(localProfile);
  latestProfile = staticProfile;
  renderPreview(staticProfile);
  dom.jsonPreview.textContent = JSON.stringify(staticProfile, null, 2);

  try {
    if (!canUseLocalApi()) {
      throw new Error("Static Pages build uses browser-side normalization.");
    }
...
function enrichStaticProfile(profile) {
  const enriched = JSON.parse(JSON.stringify(profile));
  const level = Number(enriched.totalLevel || enriched.level || 1);
  const proficiencyBonus = Math.floor((level - 1) / 4) + 2;
  const cls = findClass(enriched.class);
  const classKey = normalizeName(enriched.class);

  enriched.proficiencyBonus = proficiencyBonus;
  enriched.abilityModifiers = {};
  ABILITIES.forEach((ability) => {
    enriched.abilityModifiers[ability.key] = abilityMod(enriched.abilities?.[ability.key] ?? 10);
  });

  enriched.savingThrows = {};
  ABILITIES.forEach((ability) => {
    const modifier = enriched.abilityModifiers[ability.key] || 0;
    const proficient = Boolean(enriched.savingThrowProficiencies?.[ability.key]);
    enriched.savingThrows[ability.key] = modifier + (proficient ? proficiencyBonus : 0);
  });

  enriched.skills = {};
  SKILLS.forEach(([skill, , ability]) => {
    let total = enriched.abilityModifiers[ability] || 0;
    if (enriched.skillExpertise?.[skill]) {
      total += proficiencyBonus * 2;
    } else if (enriched.skillProficiencies?.[skill]) {
      total += proficiencyBonus;
    }
    enriched.skills[skill] = total >= 0 ? `+${total}` : String(total);
  });

  enriched.passivePerception = 10 + parseInt(enriched.skills.perception || "0", 10);
  enriched.spellSlots = buildStaticSpellSlots(cls, level);
  enriched.pactMagicSlots = buildStaticPactMagicSlots(cls, level);

  if (enriched.spellcastingByClass?.[classKey]) {
    const entry = enriched.spellcastingByClass[classKey];
    entry.preparedLimit = getStaticPreparedLimit(cls, level, enriched.abilityModifiers);
    entry.cantripsKnown = getProgressionValue(cls?.cantripProgression, level);
    entry.knownSpellLimit = getStaticKnownSpellLimit(cls, level, entry.knownSpells || []);
    entry.mysticArcanum = {};

    enriched.spellcasting = {
      mode: entry.mode,
      preparedLimit: entry.preparedLimit,
      cantripsKnown: entry.cantripsKnown,
      knownSpellLimit: entry.knownSpellLimit,
      preparedSpells: [...(entry.preparedSpells || [])],
      knownSpells: [...(entry.knownSpells || [])],
  ...
  enriched.racialFeatures = buildStaticRacialFeatures(enriched);
  enriched.selectedFeatFeatures = buildStaticSelectedFeatFeatures(enriched);
  enriched.characterFeatures = [];
  enriched.featuresByClass = {};
  enriched.classResources = {};
  enriched.classResourcesByClass = {};
  enriched.pendingSubclassSelections = [];
  enriched.pendingAsiPoints = 0;
  enriched.hitDice = {
    dieSize: Number(enriched.hitDie || cls?.hitDie || 0),
    current: level,
    max: level,
    pools: [
      {
  ...
  enriched.levelUpHistory = [];

  return enriched;
}

function buildStaticSpellSlots(cls, level) {
  const casterLevel = getStaticCasterLevel(cls, level);
  const slots = casterLevel > 0 ? { ...(FULL_CASTER_SLOTS[casterLevel] || {}) } : {};
  return {
    current: { ...slots },
    max: { ...slots },
  };
...
function buildStaticPactMagicSlots(cls, level) {
  if (String(cls?.casterProgression || "").toLowerCase() !== "pact") {
    return {
      current: 0,
      max: 0,
      slotLevel: 0,
  ...
  const pact = PACT_MAGIC_SLOTS[clamp(Number(level || 1), 1, 20)] || { slotLevel: 0, slots: 0 };
  return {
    current: pact.slots,
    max: pact.slots,
...
function getStaticCasterLevel(cls, level) {
  const progression = String(cls?.casterProgression || "none").toLowerCase();
  const safeLevel = clamp(Number(level || 1), 1, 20);

  if (progression === "full") return safeLevel;
  if (progression === "artificer") return Math.ceil(safeLevel / 2);
  if (progression === "1 2") return Math.floor(safeLevel / 2);
  if (progression === "1 3") return Math.floor(safeLevel / 3);
  return 0;
}

function getStaticPreparedLimit(cls, level, modifiers) {
  if (!cls || cls.spellcastingMode === "none") return 0;
  if (cls.spellcastingMode === "known") return 0;

  const ability = normalizeName(cls.spellcastingAbility || "");
  const abilityModifier = Number(modifiers?.[ability] || 0);
  return Math.max(1, Number(level || 1) + abilityModifier);
}

function getStaticKnownSpellLimit(cls, level, selectedKnownSpells) {
  const fixed = getProgressionValue(cls?.spellsKnownProgressionFixed, level);
  if (fixed > 0) return fixed;
  return Array.isArray(selectedKnownSpells) ? selectedKnownSpells.length : 0;
}

function getProgressionValue(progression, level) {
  if (!Array.isArray(progression)) return 0;
  return Number(progression[clamp(Number(level || 1), 1, 20) - 1] || 0);
}

function buildStaticRacialFeatures(profile) {
  const race = data.races.find((entry) => normalizeName(entry.name) === normalizeName(profile.race));
  const subrace = data.subraces.find(
    (entry) =>
      normalizeName(entry.name) === normalizeName(profile.subrace) &&
      normalizeName(entry.raceName) === normalizeName(profile.race)
  );

  return [race, subrace].flatMap((source) => extractStaticFeatures(source, source?.name || "", "racial"));
}

function buildStaticSelectedFeatFeatures(profile) {
  return (profile.selectedFeats || [])
    .map((selectedFeat) => {
      const feat = data.feats.find((entry) => normalizeName(entry.name) === normalizeName(selectedFeat.name));
      if (!feat) return null;

      const chosenLine = selectedFeat.chosenAbility
        ? `\nChosen ability increase: ${String(selectedFeat.chosenAbility).toUpperCase()}.`
        : "";

      return {
        id: `feat:${normalizeName(feat.name)}`,
        type: "selectedFeat",
        group: "Selected Feats",
        name: feat.name,
        source: feat.source || "",
        level: Number(selectedFeat.acquiredLevel || 0),
        description: flattenEntryText(feat.entries || []) + chosenLine,
      };
    })
    .filter(Boolean);
}

function extractStaticFeatures(source, group, type) {
  if (!source) return [];

  return (source.entries || [])
    .filter((entry) => {
      if (!entry?.name || !Array.isArray(entry.entries)) return false;
      return !["age", "alignment", "appearance", "creature type", "darkvision", "flight", "languages", "language", "lifespan", "size", "speed", "names"].includes(normalizeName(entry.name));
    })
    .map((entry) => ({
      id: `${type}:${normalizeName(group)}:${normalizeName(entry.name)}`,
      type: "racial",
      group,
      name: entry.name,
...
function flattenEntryText(entries) {
  const lines = [];
